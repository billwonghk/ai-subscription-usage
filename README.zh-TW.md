...
預設目錄會自動識別，報表已顯示用量時不需要設定。安裝後仍未識別到既有用戶端或記錄時，在應用程式內開啟「設定及使用說明」，把其中的完整設定提示詞複製給能在同一部電腦上檢查檔案並執行命令的本機 Codex、Claude Code、WorkBuddy、Gemini CLI 或其他 AI。不能操作這部電腦的一般網頁聊天無法完成連接。本機 AI 會診斷已安裝的應用程式，只登記白名單內既有的唯讀記錄目錄、驗證資料來源並更新一次報表。

`SETUP_WITH_AI.md` 也保存相同的完整提示詞和操作流程。AI 只能執行上述唯讀診斷與受控設定命令，不得掃描整個硬碟，不得讀取憑證或對話內容，不得修改任何 AI 用戶端，不得猜測記錄格式，也不得把不支援的資料來源聲稱為已連接。設定檔不接受命令、網路位址、憑證或任意解析程式碼。
