...
기본 폴더는 자동으로 감지됩니다. 보고서에 사용량이 표시되면 설정할 필요가 없습니다. 설치 후에도 기존 클라이언트나 기록이 감지되지 않으면 앱에서 “구성 및 사용 안내”를 열고 전체 설정 프롬프트를 같은 컴퓨터에서 파일을 확인하고 명령을 실행할 수 있는 로컬 Codex, Claude Code, WorkBuddy, Gemini CLI 또는 다른 AI에 전달하세요. 이 컴퓨터를 조작할 수 없는 일반 웹 채팅은 연결을 수행할 수 없습니다. 로컬 AI는 설치된 앱을 진단하고 허용 목록에 있는 기존 읽기 전용 로그 폴더만 등록한 뒤 데이터 소스를 검증하고 보고서를 한 번 새로 고칩니다.

`SETUP_WITH_AI.md`에도 동일한 전체 프롬프트와 절차가 있습니다. AI는 위의 읽기 전용 진단 및 제어된 설정 명령만 실행할 수 있습니다. 디스크 전체 검색, 자격 증명이나 대화 본문 읽기, AI 클라이언트 수정, 기록 형식 추측, 지원되지 않는 소스를 연결되었다고 보고하는 행위는 금지됩니다. 설정 파일은 명령, 네트워크 주소, 자격 증명 또는 임의의 파싱 코드를 허용하지 않습니다.
