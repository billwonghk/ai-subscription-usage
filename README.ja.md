...
既定のフォルダは自動検出されます。レポートに使用量が表示されている場合、設定は不要です。インストール後も既存のクライアントや記録が検出されない場合は、アプリの「設定・使用ガイド」を開き、完全な設定プロンプトを、同じコンピューター上でファイルを確認してコマンドを実行できるローカル Codex、Claude Code、WorkBuddy、Gemini CLI、または別の AI に渡してください。このコンピューターを操作できない一般的な Web チャットでは接続できません。ローカル AI はインストール済みアプリを診断し、許可された既存の読み取り専用ログフォルダだけを登録して、データソースを検証し、レポートを1回更新します。

`SETUP_WITH_AI.md` にも同じ完全なプロンプトと手順があります。AI が実行できるのは上記の読み取り専用診断と制御された設定コマンドだけです。ディスク全体のスキャン、認証情報や会話本文の読み取り、AI クライアントの変更、記録形式の推測、未対応ソースを接続済みとする報告は禁止されています。設定ファイルはコマンド、ネットワークアドレス、認証情報、任意の解析コードを受け付けません。
